...
const buildItem = (
  ...
  bureau: Bureau,
  idSuffix: string,
  values: Omit<NegativeItem, "id" | "sourceName" | "bureau">
): NegativeItem => ({
...
const parseEquifaxCollections = (sourceName: string, text: string): NegativeItem[] => {
  ...
  return accountMatches
    .map((match, index) => {
      ...
      const dateOpened =
        extractField(block, [/Assigned\s*:\s*([0-9/]+)/i, /Date of 1st Delinquency\s*:\s*([0-9/]+)/i]) || "Not listed";
      const status =
...
const parseEquifaxNegativeAccounts = (sourceName: string, text: string): NegativeItem[] => {
  ...
  for (let index = 0; index < accountLineIndexes.length; index += 1) {
    ...
    const accountNumber =
      extractField(block, [/Account Number\s*:\s*([*0-9A-Z-]+)/i]) || extractLabelValue(block, "Account Number") || "Not listed";
    ...
    const status =
      ...
      (hasChargeOffCode ? "Charge Off" : hasCollectionCode ? "Collection Account" : toNumber(amountPastDue) > 0 ? "Collection" : "Not listed");
...
export const parseNegativeItems = (inputs: ParseInput[]): NegativeItem[] => {
  ...
  for (const input of inputs) {
    ...
    const transUnionAcr = parseTransUnionAnnualCreditReport(input.sourceName, normalized);
    if (transUnionAcr.length > 0) {
      results.push(...transUnionAcr);
      continue;
    }

    const transUnionPortal = parseTransUnionPortalSnapshot(input.sourceName, normalized);
    if (transUnionPortal.length > 0) {
      results.push(...transUnionPortal);
      continue;
    }

    const transUnionSpecialized = parseTransUnionNegativeAccounts(input.sourceName, normalized);
    if (transUnionSpecialized.length > 0) {
      results.push(...transUnionSpecialized);
      continue;
    }

    const equifaxSpecialized = parseEquifaxNegativeAccounts(input.sourceName, normalized);
  ...
  const deduped = new Map<string, NegativeItem>();
  for (const item of results) {
    // Masked account numbers can collide within one bureau. The opened date
    // distinguishes separate tradelines while still merging repeated copies.
    const key = `${item.bureau}|${item.creditor}|${item.accountNumber}|${item.dateOpened}`;
    ...
    const statusScore = (value: string): number => (value !== "Not listed" ? 1 : 0);
    const balanceScore = (value: string): number => (value !== "Not listed" ? 1 : 0);
    const richness = statusScore(item.status) + balanceScore(item.balance);
    const existingRichness = statusScore(existing.status) + balanceScore(existing.balance);

    if (item.confidence > existing.confidence || richness > existingRichness) {
      deduped.set(key, item);
    }
  }
