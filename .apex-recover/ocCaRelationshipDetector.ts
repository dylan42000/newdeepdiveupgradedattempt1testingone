...
export function detectOCCARelationship(a: NegativeItem, b: NegativeItem): OCCARelationship {
  ...
  const oc = aIsCO && !aIsCA ? a : bIsCO && !bIsCA ? b : null;
  const ca = aIsCA && !aIsCO ? a : bIsCA && !bIsCO ? b : aIsCA ? a : bIsCA ? b : null;
  ...
  if ((sold || caFamily?.isCollectionAgency) && balancesOk && timelineOk) {
    return {
      decision: 'link_only',
      reason: sold
