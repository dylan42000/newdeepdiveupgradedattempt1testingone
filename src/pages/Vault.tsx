...
export function Vault() {
  const { vaultDocs, addVaultDoc, removeVaultDoc, vaultTotalSize, autopilot, updateAutopilot, personalInfo, updatePersonalInfo } = useAppContext();
  ...
  const isConfirmed = Boolean(autopilot?.identityInfoConfirmed || personalInfo?.identityInfoConfirmed);
  const evidenceStatus = evaluateEvidenceReadiness(
    ...
    "general",
    { identityInfoConfirmed: isConfirmed },
  );
  ...
  const onDrop = useCallback(async (accepted: File[]) => {
    ...
    const errs: string[] = [];
    let uploaded = 0;
    let uploadedIdentityDocId: string | undefined;
    ...
    try {
      for (const file of accepted) {
        ...
        try {
          ...
          await addVaultDoc({
            id: documentId, name: file.name, type: file.type, size: file.size,
            category, uploadDate: new Date().toISOString(), data: arrayBuffer, tags,
          });
          if (category === "Identity Proof" && !uploadedIdentityDocId) uploadedIdentityDocId = documentId;
      ...
      if (uploaded > 0) {
        if (includesIdentity) {
          updateAutopilot?.({ identityInfoConfirmed: true, identityDocId: uploadedIdentityDocId });
          updatePersonalInfo?.({ identityInfoConfirmed: true, identityDocId: uploadedIdentityDocId });
        }
        setUploadSummary(`${uploaded} document${uploaded === 1 ? "" : "s"} securely saved to the local Vault.${includesIdentity ? " Your identity evidence is now recognized and confirmed across all AutoPilot areas." : ""}`);
            ...
            <div key={doc.id} className="cyber-panel p-4 flex flex-col gap-3 hover:border-zinc-700 transition-all">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => {
                      updateAutopilot?.({ identityInfoConfirmed: true, identityDocId: doc.id });
                      updatePersonalInfo?.({ identityInfoConfirmed: true, identityDocId: doc.id });
                      setUploadSummary(`"${doc.name}" is now linked as your Primary Identity Proof globally across AutoPilot.`);
                    }}
                    className={`px-2 py-1 text-[10px] font-bold rounded border transition-colors ${
                      (autopilot?.identityDocId === doc.id || personalInfo?.identityDocId === doc.id)
                        ? "bg-emerald-950 text-emerald-300 border-emerald-700"
                        : "bg-zinc-900 text-zinc-400 border-zinc-700 hover:text-emerald-300 hover:border-emerald-700"
                    }`}
                    title="Link as Primary Identity Proof"
                  >
                    {(autopilot?.identityDocId === doc.id || personalInfo?.identityDocId === doc.id) ? "★ LINKED ID" : "LINK AS ID"}
                  </button>
