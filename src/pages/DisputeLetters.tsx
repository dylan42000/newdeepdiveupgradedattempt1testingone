...
import type { FailedDisputeLetter } from "../types/creditRepair";
...
export function DisputeLetters() {
  ...
  const [failedDisputeLetters, setFailedDisputeLetters] = useState<FailedDisputeLetter[]>(() => {
    try {
      const saved = localStorage.getItem(`dylandos_failed_letters_${activeProfileId || 'default'}`);
  ...
  useEffect(() => {
    const refresh = () => {
      setRecoveryTasks(getOpenDisputeResolutionTasks(activeProfileId || 'default'));
      try {
        const saved = localStorage.getItem(`dylandos_failed_letters_${activeProfileId || 'default'}`);
        if (saved) setFailedDisputeLetters(JSON.parse(saved));
      } catch {}
    };
    refresh();
    ...
    try {
      ...
      if (regenResult.success && (regenResult.newContent || regenResult.newHtmlContent)) {
        ...
        setFailedDisputeLetters((prev) => {
          ...
          try { localStorage.setItem(`dylandos_failed_letters_${activeProfileId || 'default'}`, JSON.stringify(next)); } catch {}
          return next;
        });
      }
    } catch { /* ignore */ }
  }, [negativeItems, personalInfo, autopilot, activeProfileId, addDisputeLetter]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Failed Dispute Letters — Immediate Redo Panel */}
      {failedDisputeLetters.length > 0 && (
        <section className="cyber-panel p-4 border-red-800/80 bg-gradient-to-r from-red-950/40 to-zinc-950 rounded-2xl shadow-xl">
          <div className="flex items-center justify-between gap-4 mb-3 flex-wrap">
            <div className="flex items-center gap-2 text-white font-bold text-sm">
              <AlertTriangle className="h-5 w-5 text-red-400" />
              <span>{failedDisputeLetters.length} Failed Dispute Letter{failedDisputeLetters.length === 1 ? '' : 's'} — Redo Needed</span>
            </div>
            <button
              type="button"
