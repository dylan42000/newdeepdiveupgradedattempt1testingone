...
import { calcPriorityScore } from "../context/AppContext";
import {
  checkOverdueItems, getDaysRemaining,
  resolveResponseOutcome, ESCALATION_LADDER, BUREAUS,
...
import { AutopilotCampaign, DisputeItemStatus, DisputeRound } from "../types";
import ScoreSimulator from "../components/ScoreSimulator";
import DisputeTimeline from "../components/DisputeTimeline";
import CrossBureauMatrix from "../components/CrossBureauMatrix";

import { AutoPilotDashboard } from "../components/AutoPilotDashboard";
import type { ResponseOutcome } from "../components/AutoPilotDashboard";
...
import type { AutoPilotSettingsV3, AutoPilotEngineState, HoldQueueEntry, FCRADeadline, PassNumber, AutoPilotCycleResult } from "../types/creditRepair";
...
const RESPONSE_OPTIONS = ["Verified", "Updated", "Deleted", "NoResponse"] as const;
type ResponseOption = typeof RESPONSE_OPTIONS[number];

function nextStatusFromResponse(outcome: ResponseOutcome, currentRound: DisputeRound): DisputeItemStatus {
  if (outcome === "Deleted") return "Won";

  if (outcome === "Verified" || outcome === "Updated") {
    if (currentRound === 1) return "Round1-Verified";
    if (currentRound === 2) return "Round2-Verified";
    if (currentRound === 3) return "Round3-Verified";
    if (currentRound === 4) return "Round4-Verified";
    if (currentRound === 5) return "Round5-Verified";
    return "Round6-PreLit";
  }

  const nextRound = Math.min(6, currentRound + 1) as DisputeRound;
  if (nextRound === 1) return "Round1-Pending";
  if (nextRound === 2) return "Round2-Pending";
  if (nextRound === 3) return "Round3-Pending";
  if (nextRound === 4) return "Round4-Legal";
  if (nextRound === 5) return "Round5-CFPB";
  return "Round6-PreLit";
}
...
export function Autopilot({ onNavigate }: { onNavigate?: (page: AppPage) => void }) {
  ...
  const handleV2RunCycle = useCallback(async () => {
    ...
    // Older/legacy dispatches store their deadline on the item rather than in
    // TimelineTracker. Promote only an expired deadline once, so a missed
    // Round 1 is prepared as Round 2 instead of being left in a dead queue.
    // This prepares a draft; it never represents that a new letter was mailed.
    const now = Date.now();
    const overdueForNextRound = negativeItems.filter((item) => {
    ...
    for (const item of overdueForNextRound) {
      ...
      if (nextPass > currentPass) {
        ...
        updateDisputeItemStatus(item.id, nextStatusFromResponse("NoResponse", item.disputeRound), nextPass as DisputeRound);
        addAutopilotLog({
    ...
    try {
      ...
      addAutopilotLog({
        message: `[Orchestrator] ${orchestrated.packets.length} packet(s), ${orchestrated.gateFailures.length} gate remediation(s), status=${orchestrated.missionControl.autopilotStatus}`,
        level: "info",
        type: "system" as const,
      });

      // ── Save V2 letters into AppContext so they survive page refresh ──
      if (result.letters.length > 0) {
        result.letters.filter((v2Letter) => {
          if (v2Letter.status === 'blocked' || v2Letter.validationErrors?.length) {
            addAutopilotLog({
              message: `[V2 BLOCKED] ${v2Letter.itemName} → ${v2Letter.targetName}: ${(v2Letter.validationErrors || []).join('; ') || 'validation failed'}`,
              level: 'error',
              type: 'system' as const,
              metadata: { letterId: v2Letter.id, itemId: v2Letter.itemId },
            });
            return false;
          }
          return true;
          ...
          const disputeLetter = {
            id: v2Letter.id,
            negativeItemIds: [v2Letter.itemId],
            // Keep queue content plaintext for downstream placeholder + export flows.
            content: v2Letter.letterContent,
            // BUG-04 FIX: Save the full formatted HTML so preview and PDF export work correctly
            htmlContent: v2Letter.htmlContent || undefined,
            letterVersion: 'v2' as const,
            createdAt: v2Letter.createdAt,
            status: "Draft" as const,
            bureau: v2Letter.targetName,
            round: (Math.min(v2Letter.passNumber, 6) as import('../types').DisputeRound),
            batchId: result!.cycleId,
            templateType: "611-Reinvestigation" as import('../types').LetterTemplateType,
            certifiedMail: autopilot.certifiedMailDefault,
            mailed: false,
            mailedAt: null,
            targetType: v2Letter.targetType,
            mailDeliveryProvider: autopilot.mailDeliveryProvider,
          };
          addDisputeLetter(disputeLetter);
        });
        addAutopilotLog({ message: `[V2] ✅ ${result.letters.length} letter(s) saved to mail queue`, level: "success", type: 'system' as const });
      }

      // ── Update item statuses in AppContext ───────────────────────────
      result.dispatchPlan.items.forEach((planItem) => {
        const pass = planItem.passNumber;
      ...
      addAutopilotLog({ message: `[V2 CYCLE] Complete — ${result.lettersGenerated} letters, ${result.itemsOnHold} on hold`, level: "success", type: 'system' as const });
      logEvent({ type: "autopilot_cycle_run", title: "AutoPilot V2 Cycle Complete", detail: `${result.lettersGenerated} letters generated, ${result.itemsOnHold} items on hold` });
    } catch (err: any) {
    ...
    setV2HoldEntries(HoldQueue.getAll(profileId));
    setV2Deadlines(TimelineTracker.getByProfile(profileId));
  }, [profileId, negativeItems, personalInfo, isPersonalInfoComplete, v2Settings, evidenceDocs, autopilot.certifiedMailDefault, autopilot.mailDeliveryProvider, addAutopilotLog, addDisputeLetter, updateDisputeItemStatus, logEvent]);
  ...
  const handleV2LogResponse = useCallback((itemId: string, outcome: ResponseOutcome, bureau: string) => {
    ...
    const newStatus = nextStatusFromResponse(outcome, item.disputeRound);
