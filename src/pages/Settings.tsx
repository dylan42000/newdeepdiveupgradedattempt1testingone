...
export function SettingsPage() {
  ...
  const handleTestGeminiKey = async () => {
    ...
    try {
      ...
      // Test both slots. Testing only Key 1 made a healthy load-balancer key
      // look unused and hid a bad primary key until a batch failed.
      const attempts = await Promise.allSettled(configuredKeys.map(async (key, index) => {
        let lastErr = "";
        for (const model of GEMINI_MODEL_CHAIN) {
          const res = await fetch("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions", {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${key}` },
            // Keep this request identical to Google's documented minimal
            // OpenAI-compatible request. A tiny max_tokens value is rejected
            // by some Gemini models before the API key is even evaluated.
  ...
  const handleTestOpenAIKey = async () => {
    ...
    try {
      ...
      const res = await fetch("https://api.openai.com/v1/responses", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${key}` },
        // Responses API requires at least 16 output tokens, even for a ping.
        body: JSON.stringify({ model: OPENAI_DEFAULT_MODEL, input: "Reply with OK", max_output_tokens: 16 }),
      });
      if (res.ok) {
