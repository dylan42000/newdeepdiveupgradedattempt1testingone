...
import {
  ...
  getHuggingFaceApiKey, setHuggingFaceApiKey,
  getDeepSeekApiKey, setDeepSeekApiKey,
  getTogetherApiKey, setTogetherApiKey,
  getMistralApiKey, setMistralApiKey,
...
export function SettingsPage() {
  ...
  const [hfKeyInput, setHfKeyInput] = useState(() => getHuggingFaceApiKey());
  const [showHfKey, setShowHfKey] = useState(false);
  const [hfSaveStatus, setHfSaveStatus] = useState<"idle" | "saved" | "error">("idle");
  const [hfTestStatus, setHfTestStatus] = useState<"idle" | "testing" | "ok" | "fail">("idle");
  ...
  // New free provider state
  const [deepSeekKeyInput, setDeepSeekKeyInput] = useState(() => getDeepSeekApiKey());
  const [deepSeekSaveStatus, setDeepSeekSaveStatus] = useState<"idle" | "saved">("idle");
  const [togetherKeyInput, setTogetherKeyInput] = useState(() => getTogetherApiKey());
  const [togetherSaveStatus, setTogetherSaveStatus] = useState<"idle" | "saved">("idle");
  const [mistralKeyInput, setMistralKeyInput] = useState(() => getMistralApiKey());
  const [mistralSaveStatus, setMistralSaveStatus] = useState<"idle" | "saved">("idle");
  ...
  const handleSaveHuggingFaceKey = () => {
    if (!hfKeyInput.trim()) { setHfSaveStatus("error"); setTimeout(() => setHfSaveStatus("idle"), 2000); return; }
    setHuggingFaceApiKey(hfKeyInput.trim());
    setHfSaveStatus("saved");
    setTimeout(() => setHfSaveStatus("idle"), 2500);
  };

  const handleTestHuggingFaceKey = async () => {
    setHfTestStatus("testing");
    setHfTestMsg("");
    try {
      const token = hfKeyInput.trim() || getHuggingFaceApiKey();
      if (!token) { setHfTestStatus("fail"); setHfTestMsg("No HuggingFace token entered."); setTimeout(() => setHfTestStatus("idle"), 4000); return; }
      const res = await fetch("https://api-inference.huggingface.co/models/meta-llama/Llama-3.2-3B-Instruct/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "meta-llama/Llama-3.2-3B-Instruct", messages: [{ role: "user", content: "Reply with OK" }], max_tokens: 16 }),
      });
      if (res.ok) {
        setHfTestStatus("ok");
        setHfTestMsg("HuggingFace connected. Free fallback ready.");
      } else {
        const err = await res.text().catch(() => "");
        setHfTestStatus("fail");
        setHfTestMsg(res.status === 429 ? "HuggingFace free tier rate-limited (HTTP 429) — try again shortly." : `HuggingFace test failed: HTTP ${res.status} ${err.slice(0, 120)}`);
      }
    } catch (e: any) { setHfTestStatus("fail"); setHfTestMsg(e.message || "Connection failed."); }
    setTimeout(() => setHfTestStatus("idle"), 5000);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="cyber-panel p-6">
        ...
        <p className="text-xs text-zinc-600 font-mono mb-4">
          Autopilot dispute letters use <span className="text-[#00ffff]">Groq×2</span> and <span className="text-[#ff9900]">Gemini</span> only. If both primary providers are rate-limited, letter jobs wait and resume automatically. Backup providers (Cloudflare, HuggingFace FREE) serve non-letter tools per your routing mode below. OpenAI and Pollinations have been retired from the routing chain.
        </p>
        <div className="mb-5">
          ...
          >
            <option value="primary-stack">PRIMARY STACK — Groq×2 → Gemini → Cloudflare → HuggingFace FREE</option>
            <option value="gemini-heavy">GEMINI-HEAVY — Gemini first (parse/long CFPB)</option>
            <option value="backup-quality">MAX QUALITY BACKUPS — Gemini → Groq → HuggingFace → CF</option>
            <option value="experimental-openai-first">FREE-FIRST — HuggingFace first (experimental, rate-limited)</option>
          </select>
        </div>
        ...
        {showModeMigrationBanner && (
          <div className="mb-5 rounded border border-[#ff9900]/40 bg-[#ff9900]/5 p-3 text-[11px] font-mono text-[#ff9900] flex items-start justify-between gap-3">
            <span>
              Provider mode updated: OpenAI + Pollinations were retired from the chain (API deprecation). Free HuggingFace fallback added as the last-resort safety net.
            </span>
            <button
              type="button"
        ...
        <h4 className="text-xs font-bold text-[#00ffff] font-mono mb-3 flex items-center gap-2 border-b border-zinc-800 pb-2">
          <Zap size={12} /> PRIMARY AI (Letters & Core)
        </h4>
        ...
        <div className="space-y-3 border-t border-zinc-800 pt-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-emerald-400 font-mono">HUGGINGFACE — FREE FALLBACK (FREE TOKEN)</span>
            <KeyStatusPill configured={!!(getHuggingFaceApiKey() || hfKeyInput.trim())} label={getHuggingFaceApiKey() || hfKeyInput.trim() ? 'HuggingFace configured' : 'HuggingFace optional'} />
          </div>
          <p className="text-[10px] text-zinc-500 font-mono">Free token from <a href="https://huggingface.co/settings/tokens" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:underline">huggingface.co/settings/tokens</a> — no credit card required. Automatically used as the last-resort safety net when every other provider fails.</p>
          <p className="text-[10px] text-zinc-600 font-mono">Serves meta-llama/Llama-3.2-3B-Instruct through the OpenAI-compatible Inference API. Free tier is rate-limited, so it is never tried first.</p>
          <div className="relative">
            <input type={showHfKey ? "text" : "password"} value={hfKeyInput} onChange={(e) => setHfKeyInput(e.target.value)} placeholder="hf_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" className="w-full bg-black/60 border border-zinc-700 rounded px-3 py-2 pr-10 text-sm font-mono text-white placeholder:text-zinc-700 focus:outline-none focus:border-emerald-400" autoComplete="off" spellCheck={false} />
            <button type="button" onClick={() => setShowHfKey(v => !v)} className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-zinc-400">{showHfKey ? <EyeOff size={14} /> : <Eye size={14} />}</button>
          </div>
          <div className="flex gap-2">
            <button onClick={handleSaveHuggingFaceKey} className={`cyber-button px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 ${hfSaveStatus === "saved" ? "border-[#00ff00] text-[#00ff00] bg-[#00ff00]/10" : hfSaveStatus === "error" ? "border-red-500 text-red-400" : "border-emerald-400 text-emerald-400 hover:bg-emerald-400/10"}`}>
              {hfSaveStatus === "saved" ? <><CheckCircle2 size={12} /> SAVED</> : hfSaveStatus === "error" ? <><AlertTriangle size={12} /> EMPTY</> : <><Key size={12} /> SAVE</>}
            </button>
            <button onClick={handleTestHuggingFaceKey} disabled={hfTestStatus === "testing"} className={`cyber-button px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 ${hfTestStatus === "ok" ? "border-[#00ff00] text-[#00ff00]" : hfTestStatus === "fail" ? "border-red-500 text-red-400" : "border-zinc-600 text-zinc-400"} disabled:opacity-50`}>
              {hfTestStatus === "testing" ? <><RefreshCw size={12} className="animate-spin" /> TESTING...</> : hfTestStatus === "ok" ? <><Wifi size={12} /> OK</> : hfTestStatus === "fail" ? <><WifiOff size={12} /> FAILED</> : <><Wifi size={12} /> TEST FREE TIER</>}
            </button>
            {hfTestMsg && <span className={`text-xs font-mono self-center ${hfTestStatus === "ok" ? "text-[#00ff00]" : "text-red-400"}`}>{hfTestMsg}</span>}
          </div>
          {hfTestMsg && <APIStatusAdvice message={hfTestMsg} />}
        </div>

        {/* ── DeepSeek (FREE) ── */}
        <div className="space-y-3 border-t border-zinc-800 pt-4 mt-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-sky-400 font-mono">DEEPSEEK — FREE (no CC needed)</span>
            <KeyStatusPill configured={!!getDeepSeekApiKey()} label={getDeepSeekApiKey() ? 'DeepSeek configured' : 'DeepSeek optional'} />
          </div>
          <div className="relative">
            <input type="password" value={deepSeekKeyInput}
              onChange={(e) => setDeepSeekKeyInput(e.target.value)}
              placeholder="sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
              className="w-full bg-black/60 border border-zinc-700 rounded px-3 py-2 pr-10 text-sm font-mono text-white placeholder:text-zinc-700 focus:outline-none focus:border-sky-400"
              autoComplete="off" />
          </div>
          <div className="flex gap-2">
            <button onClick={() => { setDeepSeekApiKey(deepSeekKeyInput.trim()); setDeepSeekSaveStatus('saved'); setTimeout(() => setDeepSeekSaveStatus('idle'), 2500); }}
              className={`cyber-button px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 ${deepSeekSaveStatus === 'saved' ? 'border-[#00ff00] text-[#00ff00] bg-[#00ff00]/10' : 'border-sky-400 text-sky-400 hover:bg-sky-400/10'}`}>
              {deepSeekSaveStatus === 'saved' ? <><CheckCircle2 size={12} /> SAVED</> : <><Key size={12} /> SAVE</>}
            </button>
            <span className="text-[10px] text-zinc-600 font-mono self-center">Get: platform.deepseek.com/api_keys</span>
          </div>
        </div>
        ...
        <div className="space-y-3 border-t border-zinc-800 pt-4 mt-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-violet-400 font-mono">TOGETHER AI — FREE ($1 credit, no CC)</span>
            <KeyStatusPill configured={!!getTogetherApiKey()} label={getTogetherApiKey() ? 'Together configured' : 'Together optional'} />
          </div>
          <div className="relative">
            <input type="password" value={togetherKeyInput}
              onChange={(e) => setTogetherKeyInput(e.target.value)}
              placeholder="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
              className="w-full bg-black/60 border border-zinc-700 rounded px-3 py-2 pr-10 text-sm font-mono text-white placeholder:text-zinc-700 focus:outline-none focus:border-violet-400"
              autoComplete="off" />
          </div>
          <div className="flex gap-2">
            <button onClick={() => { setTogetherApiKey(togetherKeyInput.trim()); setTogetherSaveStatus('saved'); setTimeout(() => setTogetherSaveStatus('idle'), 2500); }}
              className={`cyber-button px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 ${togetherSaveStatus === 'saved' ? 'border-[#00ff00] text-[#00ff00] bg-[#00ff00]/10' : 'border-violet-400 text-violet-400 hover:bg-violet-400/10'}`}>
              {togetherSaveStatus === 'saved' ? <><CheckCircle2 size={12} /> SAVED</> : <><Key size={12} /> SAVE</>}
            </button>
            <span className="text-[10px] text-zinc-600 font-mono self-center">Get: api.together.xyz/settings/api-keys</span>
          </div>
        </div>
        ...
        <div className="space-y-3 border-t border-zinc-800 pt-4 mt-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-blue-400 font-mono">MISTRAL AI — FREE (free tier)</span>
            <KeyStatusPill configured={!!getMistralApiKey()} label={getMistralApiKey() ? 'Mistral configured' : 'Mistral optional'} />
          </div>
          <div className="relative">
            <input type="password" value={mistralKeyInput}
              onChange={(e) => setMistralKeyInput(e.target.value)}
              placeholder="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
              className="w-full bg-black/60 border border-zinc-700 rounded px-3 py-2 pr-10 text-sm font-mono text-white placeholder:text-zinc-700 focus:outline-none focus:border-blue-400"
              autoComplete="off" />
          </div>
          <div className="flex gap-2">
            <button onClick={() => { setMistralApiKey(mistralKeyInput.trim()); setMistralSaveStatus('saved'); setTimeout(() => setMistralSaveStatus('idle'), 2500); }}
              className={`cyber-button px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 ${mistralSaveStatus === 'saved' ? 'border-[#00ff00] text-[#00ff00] bg-[#00ff00]/10' : 'border-blue-400 text-blue-400 hover:bg-blue-400/10'}`}>
              {mistralSaveStatus === 'saved' ? <><CheckCircle2 size={12} /> SAVED</> : <><Key size={12} /> SAVE</>}
            </button>
            <span className="text-[10px] text-zinc-600 font-mono self-center">Get: console.mistral.ai/api-key/</span>
          </div>
        </div>
      ...
      <div className="cyber-panel p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          />
          <SecurityStatusRow
            label="API Key Storage — HuggingFace"
            status={getHuggingFaceApiKey() ? "active" : "warning"}
            detail={getHuggingFaceApiKey() ? "HuggingFace free token stored (last-resort fallback)" : "Optional — add a free token for an extra fallback provider"}
          />
          <SecurityStatusRow
            label="API Key Storage — DeepSeek"
            status={getDeepSeekApiKey() ? "active" : "warning"}
            detail={getDeepSeekApiKey() ? "DeepSeek free key stored (fallback chain)" : "Optional — free key from platform.deepseek.com/api_keys"}
          />
          <SecurityStatusRow
            label="API Key Storage — Together AI"
            status={getTogetherApiKey() ? "active" : "warning"}
            detail={getTogetherApiKey() ? "Together AI free key stored (fallback chain)" : "Optional — free $1 credit at api.together.xyz/settings/api-keys"}
          />
          <SecurityStatusRow
            label="API Key Storage — Mistral AI"
            status={getMistralApiKey() ? "active" : "warning"}
            detail={getMistralApiKey() ? "Mistral AI free key stored (fallback chain)" : "Optional — free key from console.mistral.ai/api-key/"}
          />
        </div>
      </div>
