...
export function MissionControlPanel({
  ...
  const handleConfirmAllRecognition = async () => {
    setBusy(true);
    setTaskError(null);
    try {
      const recTasks = tasks.filter((t) => t.field === 'userRecognizesAccount' || !t.field);
      for (const t of recTasks) {
        if (t.caseId) {
          await AutopilotOrchestrator.answerFact({
            profileId,
            caseId: t.caseId,
            field: t.field || 'userRecognizesAccount',
            value: true,
            taskId: t.id,
          });
        }
      ...
      setTaskError('Could not confirm accounts. Please try again.');
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="space-y-4" role="region" aria-label="AutoPilot Mission Control">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-4">
          ...
          {tasks.some((t) => t.field === 'userRecognizesAccount' || !t.field) && (
            <div className="mb-3 p-2.5 rounded-lg border border-emerald-800/60 bg-emerald-950/30 flex items-center justify-between">
              <span className="text-xs text-emerald-200">Dispute all imported accounts as yours?</span>
              <button
                type="button"
                disabled={busy}
                onClick={handleConfirmAllRecognition}
                className="text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-black px-3 py-1.5 rounded-lg transition-colors"
              >
                1-Click Confirm All
              </button>
            </div>
