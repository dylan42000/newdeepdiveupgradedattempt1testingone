...
export function enhanceWithMetro2Compliance(
  ...
  Object.entries(keyToItems).forEach(([key, group]) => {
    ...
    group.forEach(it => {
      ...
      if (idx >= 0) {
        enhancedItems[idx].metro2Violations = comparison.violations.map((v, i) => ({
          id: `m2-cross-${v.field}-${i}`,
          type: 'FORMATTING_DISCREPANCY' as const,
          field: v.field,
          description: v.description,
          severity: v.severity,
          bureausInvolved: v.bureausInvolved,
          legalBasis: 'FCRA § 607(b) & Metro 2 Format Guide',
          disputeLanguage: `Inconsistent reporting across bureaus for field ${v.field}: ${v.description}`,
        })) as any;
