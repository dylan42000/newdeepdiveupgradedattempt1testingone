...
export function evaluateGates(params: {
  ...
  const { profileId, item, caseRecord, vaultDocs, missingFacts } = params;
  ...
  const evidence = evaluateEvidenceReadiness(vaultDocs, inferDisputeType(item));
  results.push({
    gate: 'evidence',
...
export const CasePlanService = {
  async getPlan(id: string): Promise<CasePlan | undefined> {
  ...
  async buildPlan(params: {
    ...
    const { profileId, caseRecord, item, vaultDocs = [], passNumbers = {} } = params;
