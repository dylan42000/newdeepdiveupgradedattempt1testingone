import { routeAIRequest, AIProviderCooldownError } from './aiRouter';
...
import { renderDeterministicDisputeLetter, renderSafeFallbackDisputeLetter } from './deterministicLetterRenderer';
...
${DISPUTE_PROMPT_SYSTEM_POLICY}`;
    ...
    try {
      ...
      console.warn(`[LetterGenV2] All AI providers failed for ${taskId}, falling back to deterministic renderer: ${aiError.message}`);
      // Deterministic fallback — guarantees a letter is always produced
      const fallback = renderSafeFallbackDisputeLetter(req);
      return fallback;
    }
    ...
    // Models occasionally add a plausible but unsupported dollar amount, account
    // suffix, or case citation. Repair those narrow defects deterministically
    // before rejecting an otherwise valid consumer letter.
    if (req.item) cleanBody = repairGroundingDefects(cleanBody, req.item);
    cleanBody = ensureMinimumConsumerLetterLength(cleanBody, 200);
    ...
    // Apex L5 — anti-fabrication / UPL hard gate. Only unresolved material
    // fabrication remains a hard stop after the deterministic repair above.
    if (req.item) {
    ...
    // Ensure the letter meets minimum quality even from AI
    if (cleanBody.length < 100) {
      console.warn(`[LetterGenV2] AI produced too-short letter for ${taskId}, using deterministic fallback`);
      const fallback = renderSafeFallbackDisputeLetter(req);
      return fallback;
    }
...
function ensureMinimumConsumerLetterLength(text: string, minChars: number): string {
  if (text.trim().length >= minChars) return text.trim();
  return `${text.trim()}\n\nPlease conduct a reasonable investigation of the disputed information, correct or delete any information that cannot be verified as accurate, and send me the results in writing.`;
}

/** Remove only ungrounded generated details; never manufacture a replacement fact. */
function repairGroundingDefects(text: string, item: NegativeItem): string {
  const knownDigits = `${item.fullAccountNumber || ''} ${item.accountNumber || ''}`.replace(/\D/g, '');
  const knownSuffix = knownDigits.length >= 3 ? knownDigits.slice(-4) : '';
  const knownBalances = [item.balance, item.originalBalance, item.creditLimit]
    .filter((value): value is number => value != null)
    .map(value => Math.round(value * 100) / 100);

  let repaired = text
    // Strip only case reporters/case names; statutory citations remain intact.
    .replace(/\b\d{1,3}\s+[A-Z][a-z]+\.?\s+\d{1,4}\b|\b[A-Z][a-zA-Z]+\s+v\.\s+[A-Z][a-zA-Z]+\b|\bF\.\s?(?:Supp|3d)\b/g, '')
    .replace(/\s{2,}/g, ' ');

  repaired = repaired.replace(/(account[^\d\n]{0,24})(\d{4})\b/gi, (_match, prefix: string, digits: string) => {
    if (knownSuffix && (knownSuffix.endsWith(digits) || digits.endsWith(knownSuffix))) return `${prefix}${digits}`;
    return knownSuffix ? `${prefix}${knownSuffix}` : `${prefix}identified in my credit report`;
  });
  repaired = repaired.replace(/((?:ending\s+in|last\s+4|x{2,}|\*{2,}|#{2,})\s*)(\d{3,4})\b/gi, (_match, prefix: string, digits: string) => {
    if (knownSuffix && (knownSuffix.endsWith(digits) || digits.endsWith(knownSuffix))) return `${prefix}${digits}`;
    return knownSuffix ? `${prefix}${knownSuffix}` : 'the account identified in my credit report';
  });
  repaired = repaired.replace(/\$\s*([0-9]{1,3}(?:,[0-9]{3})*(?:\.\d{2})?|[0-9]+(?:\.\d{2})?)/g, (match, raw: string) => {
    const amount = Number(raw.replace(/,/g, ''));
    return knownBalances.some(value => Math.abs(value - amount) <= 1) ? match : 'the reported balance';
  });
  return repaired.replace(/\s+([,.;:])/g, '$1').replace(/\n{3,}/g, '\n\n').trim();
}
