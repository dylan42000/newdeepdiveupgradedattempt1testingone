...
import { findFurnisherAddress, findFurnisherAddressWithContacts, formatFurnisherAddress } from '../data/furnisherAddresses';
...
export async function enrichTargetAddress(
  ...
  const vaultHit = findFurnisherAddressWithContacts(creditorName);

  if (vaultHit) {
    const enriched = formatFurnisherAddress(vaultHit);
    console.info(
      `[AddressEnrichment] ✅ Vault/Library match found for "${creditorName}" → "${vaultHit.legalName}". ` +
