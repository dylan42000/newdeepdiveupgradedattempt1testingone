...
export interface DisputeResolutionTask {
  ...
  createdAt: string;
  updatedAt: string;
  /** Informational retry history shown in the AutoPilot recovery queue. */
...
/** Keep a recovery task open until a replacement letter is successfully created. */
export function markDisputeResolutionTaskResubmitted(taskId: string): void {
  const tasks = read();
  const task = tasks.find((entry) => entry.id === taskId && entry.status === 'open');
  if (!task) return;
  const now = new Date().toISOString();
  task.resubmitCount = (task.resubmitCount ?? 0) + 1;
  task.lastResubmittedAt = now;
  task.updatedAt = now;
  write(tasks);
}
