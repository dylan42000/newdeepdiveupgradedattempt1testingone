...
const GOV_ID_CATEGORIES = ['photo-id', 'government-id', 'drivers-license', 'driver-license', 'passport', 'state-id', 'id', 'identity-proof', 'identity proof'];
...
export function evaluateEvidenceReadiness(
  ...
  const hasGovId = hasCategory(vaultDocs, GOV_ID_CATEGORIES);
  if (hasGovId) {
    score += 35;
    const idDocs = docsWithCategory(vaultDocs, GOV_ID_CATEGORIES);
    letterModifiers.push(
      `Consumer has provided government-issued photo ID (${idDocs[0].name ?? idDocs[0].category}). Reference FACTA §605B which requires bureaus to accept identity documentation.`
    );
  } else {
