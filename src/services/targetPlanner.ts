...
import { BUREAU_DISPUTE_ADDRESSES, findFurnisherAddress as lookupFurnisherAddress } from '../data/furnisherAddresses';
...
export const TargetPlanner = {
  planTargets(
    ...
    if (shouldAddFurnisher && item.creditorName) {
      const furnisher = lookupFurnisherAddress(item.creditorName);
  ...
  planAllTargets(
    ...
    return items.map(item => {
      ...
      return this.planTargets(item, passNumber, dualTargetMode);
    });
  },
};
