...
export function planApexItemStrategy(params: {
  item: NegativeItem;
  pass: PassNumber;
  ...
  // Outcome learning nudge (baseline store exposes recommendedStrategy, not recommendAngle)
  const learnedStats = OutcomeLearningStore.getStats(item.creditorName, bureau);
  if (learnedStats.recommendedStrategy && campaignType === 'dispute') {
    primaryAngle = learnedStats.recommendedStrategy;
    confidenceFactors.push(`Outcome learning prefers angle: ${learnedStats.recommendedStrategy}`);
  }
...
/** @deprecated Use planApexItemStrategy — kept for transcript-era imports */
export const planItemStrategy = planApexItemStrategy;
