...
import { evaluateEvidenceReadiness, hasGovIdDocument, EvidenceDoc, DisputeType } from './evidenceGateService';
...
import { findFurnisherAddressWithContacts } from '../data/furnisherAddresses';
import { AddressResearchAgent } from './addressResearchAgent';
...
class AutoPilotEngineV2Class {
  ...
  async runCycle(params: {
    ...
    try {
      ...
      let contacts: any[] = [];
      try {
        const saved = typeof localStorage !== 'undefined' ? localStorage.getItem('dylandos_contacts_v1') || localStorage.getItem('dylandos_contacts') : null;
        if (saved) contacts = JSON.parse(saved);
      } catch {}

      for (const item of preFlightSelectedForTargets) {
        if (item.creditorName && (settings.dualTargetMode || (passNumbers[item.id] ?? 1) >= 2)) {
          const existing = findFurnisherAddressWithContacts(item.creditorName, item, contacts);
          if (!existing) {
            progress(`🔍 AI auto-fetching dispute address for "${item.creditorName}"...`);
            try {
              await AddressResearchAgent.searchAndVaultAddress(item.creditorName);
            } catch (e) {
              console.warn(`[AutoPilotEngineV2] AI auto-fetch address failed for ${item.creditorName}`, e);
            }
          }
        }
      }

      const targetPlans = TargetPlanner.planAllTargets(preFlightSelectedForTargets, passNumbers, settings.dualTargetMode, contacts);
      ...
      for (const planItem of planItems) {
        ...
        if (vaultDocs) {
          ...
          const identityConfirmed = Boolean(
            s.identityInfoConfirmed ||
            (personalInfo as any)?.identityInfoConfirmed ||
            s.identityDocId ||
            hasGovIdDocument(vaultDocs, s.identityDocId)
          );
          const gate = evaluateEvidenceReadiness(vaultDocs, disputeType, {
            identityInfoConfirmed: identityConfirmed,
            identityDocId: s.identityDocId,
          });
          if (!gate.canProceed) {
  ...
  async handleResponse(params: {
    ...
    if (outcome === 'deleted') {
      ...
      // FCRA failure to investigate — immediate escalation & release hold
      HoldQueue.releaseFromHold(profileId, itemId);
