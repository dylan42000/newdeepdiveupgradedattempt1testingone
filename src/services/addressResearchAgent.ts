...
/**
 * Researches an address only after the verified local address book misses.
 * AI output is saved as a review candidate, never silently trusted or mailed to.
 */
  ...
  async searchAndVaultAddress(creditorName: string): Promise<FurnisherAddress | null> {
    ...
    try {
      ...
      savePending({
        ...
        warning: 'AI-researched address requires user verification before approval, export, or mailing.',
      });
      // Deliberately block. The user must confirm/save the candidate in Address Book.
      return null;
    } catch (error) {
