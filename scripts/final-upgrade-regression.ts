...
assert(!letterGeneratorSource.includes('renderDeterministicDisputeLetter'), 'AutoPilot letters must never use the deterministic fallback renderer.');
assert(!directFurnisherSource.includes('consumer_fallback'), 'Direct-furnisher letters must never silently substitute a generic fallback.');
